...
namespace LibraryCRUD.Controllers
{
    ...
    public class BookController : Controller
    {
        ...
        public BookController(AppDbContext context, IToastNotification toastNotification, UserManager<IdentityUser> userManager, SignInManager<IdentityUser> signInManager)
        {
            _context = context;
        ...
        {
            var books = await _context.books.Include(b => b.Author).Include(b => b.Category).Include(b=>b.FavoritByUsers).OrderByDescending(o=>o.Rate).ToListAsync();
            if(User.Identity.IsAuthenticated)
            {
                var userName = User.Identity.Name;
                var user = await _userManager.FindByNameAsync(User.Identity.Name);
                var userBook = await _context.userAccounts.SingleOrDefaultAsync(u => u.Id == Convert.ToInt32(user.Id));
                ViewBag.userId = userBook.Id;
            }
